...
class NodeEmbeddingLoader:
    def __init__(self, data_dir: str = '/home/wangtao/src/kg4rd/data/data_feature'):
        ...
        self.node_types = {
            'gene/protein': ['gene_desc_embed.npz', 'protein_seq_embed.npz'],
    ...
    def load_node_embeddings(self, nodes: pd.DataFrame, device: str) -> dict[int, dict[str, torch.Tensor]]:
        node_embeddings = {}
            ...
            for embed_file in embedding_files:  # 多个嵌入文件
                ...
                for _, node_row in tqdm(type_nodes.iterrows(), total=type_nodes.shape[0], desc=f"{node_type}:{embed_file}"):  # 该类型下的所有 node
                    ...
                    if actual_id in id_to_embedding:
                        ...
                        embed_type = embed_file.replace('.npz', '').replace('_embed', '')
                        node_embeddings[node_index][embed_type] = torch.tensor(id_to_embedding[actual_id], dtype=torch.float32).to(device)

        return node_embeddings  # {node_index: {embedding_type: embedding}} 不包含 pathway
